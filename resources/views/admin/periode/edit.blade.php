...
@section('title', 'Edit Periode Survei - SKM Sumenep')
        ...
        <form action="{{ route('admin.periode.update', $periode->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" 
                       placeholder="Contoh: Triwulan I 2024" value="{{ old('nama', $periode->nama) }}" required>
                @error('nama')
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    ...
                    <select name="tahun" class="form-select @error('tahun') is-invalid @enderror" required>
                        ...
                        @for($i = date('Y'); $i >= 2020; $i--)
                            <option value="{{ $i }}" {{ old('tahun', $periode->tahun) == $i ? 'selected' : '' }}>
                                {{ $i }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="col-md-6 mb-4">
                    ...
                    <select name="triwulan" class="form-select @error('triwulan') is-invalid @enderror" required>
                        ...
                        <option value="1" {{ old('triwulan', $periode->triwulan) == 1 ? 'selected' : '' }}>I (Jan - Mar)</option>
                        <option value="2" {{ old('triwulan', $periode->triwulan) == 2 ? 'selected' : '' }}>II (Apr - Jun)</option>
                        <option value="3" {{ old('triwulan', $periode->triwulan) == 3 ? 'selected' : '' }}>III (Jul - Sep)</option>
                        <option value="4" {{ old('triwulan', $periode->triwulan) == 4 ? 'selected' : '' }}>IV (Okt - Des)</option>
                    </select>
                    @error('triwulan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-4">
                <div class="form-check">
                    <input type="checkbox" name="is_active" class="form-check-input" id="isActive" value="1" {{ old('is_active', $periode->is_active) ? 'checked' : '' }}>
